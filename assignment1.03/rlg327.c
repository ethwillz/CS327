...
void non_tunneling(dungeon_t *d, int playerY, int playerX){
  static non_tunnel_t nt[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t heap;
  int y, x;
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if (d->hardness[y][x] == 0) {
        nt[y][x].hn = heap_insert(&heap, &nt[y][x]);
      } else {
        nt[y][x].hn = NULL;
      }
    }
  ...
  while ((p = heap_remove_min(&heap))) {
    ...
    if ((nt[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) &&
        (nt[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost >
         p->cost + 1)) {
      nt[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost =
        p->cost + 1;
      nt[p->pos[dim_y] - 1][p->pos[dim_x] - 1].from[dim_y] = p->pos[dim_y];
      nt[p->pos[dim_y] - 1][p->pos[dim_x] - 1].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&heap, nt[p->pos[dim_y] - 1]
                                           [p->pos[dim_x] - 1].hn);
    }
    if ((nt[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (nt[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         p->cost + 1)) {
      nt[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
        p->cost + 1;
      nt[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_y] = p->pos[dim_y];
      nt[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&heap, nt[p->pos[dim_y] - 1]
                                           [p->pos[dim_x]    ].hn);
    }
    if ((nt[p->pos[dim_y] - 1][p->pos[dim_x] + 1].hn) &&
    ...
    if ((nt[p->pos[dim_y]    ][p->pos[dim_x] + 1].hn) &&
        (nt[p->pos[dim_y]    ][p->pos[dim_x] + 1].cost >
         p->cost + 1)) {
      nt[p->pos[dim_y]    ][p->pos[dim_x] + 1].cost =
        p->cost + 1;
      nt[p->pos[dim_y]    ][p->pos[dim_x] + 1].from[dim_y] = p->pos[dim_y];
      nt[p->pos[dim_y]    ][p->pos[dim_x] + 1].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&heap, nt[p->pos[dim_y]    ]
                                           [p->pos[dim_x] + 1].hn);
    }
    if ((nt[p->pos[dim_y] + 1][p->pos[dim_x] + 1].hn) &&
        (nt[p->pos[dim_y] + 1][p->pos[dim_x] + 1].cost >
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
	...
	printf("%c", '@');
      else if(nt[y][x].cost != INT_MAX)
	printf("%d", nt[y][x].cost % 10);
      else
      printf("%c", ' ');
    }
    printf("\n");
...
void tunneling(dungeon_t *d, int playerY, int playerX){
  static tunnel_t t[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((t[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) &&
      ...
      t[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost =
        p->cost + cost;
      t[p->pos[dim_y] - 1][p->pos[dim_x] - 1].from[dim_y] = p->pos[dim_y];
      t[p->pos[dim_y] - 1][p->pos[dim_x] - 1].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&h, t[p->pos[dim_y] - 1]
                                           [p->pos[dim_x] - 1].hn);
    }
    if ((t[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (t[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         p->cost + cost)) {
      t[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
        p->cost + cost;
      t[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_y] = p->pos[dim_y];
      t[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&h, t[p->pos[dim_y] - 1]
                                           [p->pos[dim_x]    ].hn);
    }
    if ((t[p->pos[dim_y] - 1][p->pos[dim_x] + 1].hn) &&
    ...
    if ((t[p->pos[dim_y]    ][p->pos[dim_x] + 1].hn) &&
        (t[p->pos[dim_y]    ][p->pos[dim_x] + 1].cost >
         p->cost + cost)) {
      t[p->pos[dim_y]    ][p->pos[dim_x] + 1].cost =
        p->cost + cost;
      t[p->pos[dim_y]    ][p->pos[dim_x] + 1].from[dim_y] = p->pos[dim_y];
      t[p->pos[dim_y]    ][p->pos[dim_x] + 1].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&h, t[p->pos[dim_y]    ]
                                           [p->pos[dim_x] + 1].hn);
    }
    if ((t[p->pos[dim_y] + 1][p->pos[dim_x] + 1].hn) &&
        (t[p->pos[dim_y] + 1][p->pos[dim_x] + 1].cost >
