...
void generateRooms(Dungeon *dungeon){
  ...
  for(i = 0; i < 105; i++){
    for(j = 0; j < 160 ; j++){
      Space aspace;
      aspace.material = ' ';
      dungeon->spaces[i][j] = aspace;
      if(i == 0 || i == 104 || j == 0 || j == 159)
	dungeon->spaces[i][j].hardness = 255;
      else
	dungeon->spaces[i][j].hardness = rand() % 254 + 1;
    }
  }
  ...
  for(k = 0; k < 18; k++){
	...
	for(m = leftx; m < leftx + width; m++){
	  if(dungeon->spaces[l][m].material == '.' || dungeon->spaces[l+1][m].material == '.' || dungeon->spaces[l-1][m].material == '.' || dungeon->spaces[l][m+1].material == '.' || dungeon->spaces[l][m-1].material == '.'){
	    wegood = 0;
	  }
	}
      }
	...
	for(o = leftx; o < leftx + width; o++){
	  dungeon->spaces[n][o].material = '.';
	  dungeon->spaces[n][o].hardness = 0;
	}
      }
      struct room aroom;
...
void connect(int num, Dungeon *dungeon){
  ...
  while(xcpy != closest.x || ycpy != closest.y){
    ...
    if(xcpy < closest.x && ycpy < closest.y){
      if(dungeon->spaces[ycpy][xcpy+1].hardness < dungeon->spaces[ycpy+1][xcpy].hardness)
        insertRight(&xcpy, &ycpy, dungeon);
      else
        insertDown(&xcpy, &ycpy, dungeon);
    }
    //Next room is down and to the left with relation to current room
    else if(xcpy > closest.x && ycpy < closest.y){
      if(dungeon->spaces[ycpy][xcpy-1].hardness < dungeon->spaces[ycpy+1][xcpy].hardness)
        insertLeft(&xcpy, &ycpy, dungeon);
      else
        insertDown(&xcpy, &ycpy, dungeon);
    }
...
void printDungeon(Dungeon *dungeon){
  ...
  for(i = 0; i < 105; i++){
    for(j = 0; j < 160; j++){
      printf("%c", dungeon->spaces[i][j].material);
    }
    printf("\n");
  }
}
